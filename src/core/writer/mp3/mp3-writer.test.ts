...
import { DefaultMetadataSeparator } from '../../core-config'
...
test('MP3 Metadata Write', async () => {
  ...
  expect(tag.artist).toEqual(metadata.artists.join(DefaultMetadataSeparator))
  expect(tag.performerInfo).toEqual(metadata.albumArtists!.join(DefaultMetadataSeparator))
